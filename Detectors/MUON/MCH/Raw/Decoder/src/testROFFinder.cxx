...
BOOST_AUTO_TEST_CASE(TwoDigitsInOneROF)
{
  ...
  ROFFinder rofFinder;
  rofFinder.setFirstTForbit(orbit1);
  rofFinder.process(digits);

  const auto& rofDigits = rofFinder.getDigits();
...
BOOST_AUTO_TEST_CASE(TwoDigitsInTwoROFs)
{
  ...
  const auto rofDigit1 = rofDigits[rofRecords[0].getFirstIdx()];
  const auto rofDigit2 = rofDigits[rofRecords[1].getFirstIdx()];

  BOOST_CHECK_EQUAL(digits[0], *rofDigit1);
  BOOST_CHECK_EQUAL(digits[1], *rofDigit2);
...
BOOST_AUTO_TEST_CASE(TwoDigitsInTwoROFsConsecutiveOrbits)
{
  ...
  BOOST_CHECK_EQUAL(digits[1], *rofDigit1);
  BOOST_CHECK_EQUAL(digits[0], *rofDigit2);
