                           ...
                           std::string_view inputIRFrameDataDescription)
{
  ...
  if (enableUnfilteredOutput) {
    std::string outputUnfiltered = fmt::format("urofs:MCH/{}/0", outputUnfilteredDigitRofDataDescription.data());
    auto matchersUnfiltered = select(outputUnfiltered.c_str());
    for (auto& matcher : matchersUnfiltered) {
