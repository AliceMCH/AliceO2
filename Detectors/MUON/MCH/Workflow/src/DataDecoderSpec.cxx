...
class DataDecoderTask
{
 public:
  ...
  void decodeTF(framework::ProcessingContext& pc)
  {
    ...
    mDecoder->setFirstTForbit(dh->firstTForbit);
    mRofFinder.setFirstTForbit(dh->firstTForbit);
    if (mDebug) {
      std::cout << "[DataDecoderSpec::run] first orbit is " << dh->firstTForbit << std::endl;
    }
  ...
  void decodeReadout(const o2::framework::DataRef& input)
  {
    ...
    mDecoder->setFirstTForbit(orbit);
    mRofFinder.setFirstTForbit(orbit);
  ...
  void run(framework::ProcessingContext& pc)
  {
    ...
    for (auto&& input : pc.inputs()) {
      //std::cout<<"binding: " << input.spec->binding << std::endl;
      if (input.spec->binding == "readout") {
        decodeReadout(input);
      }
    ...
    mRofFinder.process(mDecoder->getDigits(), mDummyROFs);

    if (mCheckROFs) {
      if (mRofFinder.isRofTimeMonotonic() == false) {
        LOG(ERROR) << "WARNING - ROF time does not increase monotonically";
      }
      if (mRofFinder.isDigitsTimeAligned() == false) {
        LOG(ERROR) << "WARNING - some digits have misaligned time stamps";
      }
    }
    ...
    char* digitsBuffer = mRofFinder.storeDigits(digitsSize);
    char* rofsBuffer = mRofFinder.storeROFRecords(rofsSize);
    ...
    pc.outputs().adoptChunk(Output{header::gDataOriginMCH, "ROFS", 0}, rofsBuffer, rofsSize, freefct, nullptr);
...
o2::framework::DataProcessorSpec getDecodingSpec(std::string inputSpec)
{
  ...
  return DataProcessorSpec{
    ...
    Outputs{OutputSpec{header::gDataOriginMCH, "DIGITS", 0, Lifetime::Timeframe},
            OutputSpec{header::gDataOriginMCH, "ROFS", 0, Lifetime::Timeframe},
