...
class FileReaderTask
{
 public:
  ...
  void run(framework::ProcessingContext& pc)
  {
    ...
    if (mPrint) {
      std::cout << "header_version=" << (int)CRUh.header_version << std::endl;
    }
    ...
    if (mPrint) {
      std::cout << "RDH_BLOCK_SIZE=" << RDH_BLOCK_SIZE << std::endl;
    }
...
o2::framework::DataProcessorSpec getFileReaderSpec()
{
  return DataProcessorSpec{
    ...
    Options{{"infile", VariantType::String, "data.raw", {"input file name"}},
            {"nframes", VariantType::Int, 0, {"number of frames to process"}},
