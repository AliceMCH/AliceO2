  ...
  while (vertexChainReader.Next() && mftTrackChainReader.Next() && mftClusterChainReader.Next()) {
    ...
    for (const auto& oneRof : *mftTracksROF) { // track ROF loop
      ...
      for (int iTrack = firstTrackIndex; iTrack <= lastTrackIndex; iTrack++) {
        ...
        // select tracks on the upper-right part of MFT
        //if (oneTrack.getY() < 8 || oneTrack.getX() < 0) {
        //  continue;
        //}
        ...
        //auto oneTrackAtVertex = oneTrack;
        // extrapolate track to reference plane
        //oneTrackAtVertex.propagateParamToZlinear(pvz);
        //std::cout << std::format("MFT track at vertex: z={:0.3f}  dx={:0.3f}  dy={:0.3f}", pvz, (oneTrackAtVertex.getX() - pvx), (oneTrackAtVertex.getY() - pvy)) << std::endl;
        ...
        std::array<double, 5> mchPar;
        std::array<double, 15> mchCov;
